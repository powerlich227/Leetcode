...
public class PacificAtlanticWaterFlow {
	...
	public List<List<Integer>> pacificAtlantic(int[][] heights) {
		...
		for (int i = 0; i < m; i++) {
			dFS(heights, -1, i, 0, pac);
			dFS(heights, -1, i, n - 1, atl);
		}
		for (int j = 0; j < n; j++) {
			dFS(heights, -1, 0, j, pac);
			dFS(heights, -1, m - 1, j, atl);
		}
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				if (pac[i][j] && atl[i][j])
	...
	void dFS(int[][] heights, int h, int i, int j, boolean[][] visited) {
		int m = heights.length, n = heights[0].length;
		if (i < 0 || j < 0 || i >= m || j >= n)
			return;
		if (visited[i][j] || h > heights[i][j])
			return;
		visited[i][j] = true;
